...
def _optional_sample_list(value: Any, field: str, row_id: Any, expected_len: int) -> list[Any] | None:
    ...
    if len(items) != expected_len:
        raise ValueError(f"Row {row_id!r} field {field!r} has {len(items)} samples, expected {expected_len}")
    return items
...
def flatten_limit_of_rlvr_rows(
    ...
    *,
    ...
    for row_index, row in enumerate(rows):
        ...
        finish_reasons = _as_list(row.get("finish_reason", [None] * len(predictions)), "finish_reason", problem_id)
